...
export default function Reservations() {
    ...
    const handleEdit = async (reservationId) => {
        if (window.confirm("Are you sure you want to edit this reservation?")) {
            ...
            try {
                const response = await fetch(`http://localhost:5050/reservations/update/${reservationId}`, {
                    method: 'PUT',
                    headers: {
    ...
    const handleDelete = async (reservationId) => {
        if (window.confirm("Are you sure you want to delete this reservation?")) {
            try {
                const response = await fetch(`http://localhost:5050/reservations/delete/${reservationId}`, {
                    method: 'DELETE'
                });
